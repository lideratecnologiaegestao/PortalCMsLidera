...
/** Fonte de um item no calendário. 'agenda' é editável; as demais são read-only (overlay). */
...
export class AgendaService {
   ...
   * sobrepondo as fontes read-only (sessões plenárias, eventos institucionais).
   */
  async listar(
    deISO: string,
    ateISO: string,
    ...
    const fontes = opts.fontes ?? ['agenda', 'sessao', 'evento'];
    ...
    // ── 2) Overlay: sessões plenárias (read-only) ───────────────────────────
    if (fontes.includes('sessao')) {
      const sessoes = await this.prisma.db.sessao.findMany({
        where: { dataHora: { gte: de, lte: ate } },
      ...
      for (const s of sessoes as any[]) {
        ...
        out.push({
          id: `sessao:${s.id}`, fonte: 'sessao', editavel: false, tipo: 'sessao',
          titulo: s.titulo, local: s.local ?? null, inicio: s.dataHora.toISOString(),
          fim: null, diaInteiro: false, cor: '#dc3545', descricao: null, link: `/sessoes/${s.id}`,
        });
      }
    }

    // ── 3) Overlay: eventos institucionais (read-only) ──────────────────────
    if (fontes.includes('evento')) {
      const eventos = await this.prisma.db.evento.findMany({
        where: { dataHora: { lte: ate }, OR: [{ dataFim: { gte: de } }, { dataFim: null, dataHora: { gte: de } }] },
        select: { id: true, titulo: true, tipo: true, dataHora: true, dataFim: true, local: true, slug: true },
        orderBy: { dataHora: 'asc' },
      });
      for (const e of eventos as any[]) {
        out.push({
          id: `evento:${e.id}`, fonte: 'evento', editavel: false, tipo: e.tipo ?? 'evento',
          titulo: e.titulo, local: e.local ?? null, inicio: e.dataHora.toISOString(),
          fim: e.dataFim ? e.dataFim.toISOString() : null, diaInteiro: false, cor: '#0dcaf0',
          descricao: null, link: e.slug ? `/eventos/${e.slug}` : null,
        });
      }
    }
