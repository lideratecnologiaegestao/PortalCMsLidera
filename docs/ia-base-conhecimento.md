...
> (RLS): cada câmara tem o seu próprio conhecimento, isolado. Migrations 053 e 054.
>
...
responde **ancorado no conteúdo oficial** daquela câmara. A recuperação de contexto
é montada em **3 camadas**, da mais autoritativa à mais ampla:

| Camada | Fonte | Como entra no contexto |
|---|---|---|
| **1. Fatos da entidade** | `tenants` (nome/UF), `secretarias` (comissões/setores: nome/responsável/horário/contato), DPO (`dpo_nome/email`) | **Sempre** injetado, como "INFORMAÇÕES OFICIAIS DA ENTIDADE" — o bot afirma nome da cidade, telefones, horários com confiança. Cache por tenant (TTL 5 min). |
...
| **3. RAG multi-fonte** | CMS (páginas/blocos), **Serviços**, **Notícias**, **Comissões/Setores**, **Documentos do SIC** (full-text do conteúdo extraído) | Busca FTS em todas as fontes do tenant → "CONTEÚDO DO PORTAL" com citações (título/URL). |
...
O gestor (GESTOR/ADMIN_PREFEITURA — o Administrador da Câmara) cadastra **pergunta + resposta oficial**, com:
...
conversacional ("Como falo com a Comissão de Finanças? Tem telefone?") casa com o conteúdo que tem
"comissão" e "telefone", sem exigir todas as palavras (o `plainto_tsquery`, que usa AND,
falhava nesses casos). Ordenação por `ts_rank`.

## Multi-tenant — pronto para novas entidades

Ao provisionar uma nova câmara, o bot **já nasce sabendo** sobre ela, sem nenhuma
...
- **Corpus unificado** `ia_chunks` (migration 054): chunks de TODAS as fontes (CMS,
  serviços, notícias, comissões/setores, documentos e a própria base de conhecimento) +
...
> embeddings é baixo (poucos centavos por reindexação de um portal de câmara típico).
