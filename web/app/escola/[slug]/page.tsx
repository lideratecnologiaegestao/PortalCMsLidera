/**
 * Página pública: detalhe do curso da Escola Legislativa (descrição, módulos,
...
export async function generateMetadata({ params }: { params: { slug: string } }): Promise<Metadata> {
  ...
  return {
    title: c.titulo,
    description: c.resumo ?? `Curso ${c.titulo} da Escola Legislativa da Câmara Municipal.`,
  ...
  return (
    <main className="mx-auto max-w-4xl px-4 py-8">
      <nav aria-label="Trilha" className="text-sm text-muted-fg">
        <Link href="/escola" className="underline">Escola Legislativa</Link> <span aria-hidden>›</span> {c.titulo}
      </nav>
