...
/**
 * Admin — Agenda Legislativa (calendário unificado).
 ...
 * sessões plenárias e eventos institucionais. CRUD dos itens próprios.
 */
...
// Tipos oferecidos no formulário da câmara (a agenda própria não cria 'sessao').
const TIPOS_FORM = TIPOS_AGENDA.filter((t) => t.v !== 'sessao');
...
export default function AgendaAdminPage() {
  ...
  return (
    <div className="space-y-4">
      <AdminHeader title="Agenda Legislativa" description="Calendário de eventos, reuniões, feriados, pontos facultativos, datas comemorativas e programações. Sessões plenárias e eventos aparecem no calendário (somente leitura).">
      ...
      {detalhe && (
        <Modal open onClose={() => setDetalhe(null)} title={detalhe.titulo}>
          <div className="space-y-2 text-sm">
            <p><strong>Tipo:</strong> {tipoLabel(detalhe.tipo)} <span className="text-fg/50">({detalhe.fonte === 'sessao' ? 'Sessão plenária' : 'Evento'} — somente leitura)</span></p>
