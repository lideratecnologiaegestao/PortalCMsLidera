# Portal da Câmara — Plataforma SaaS legislativa multi-tenant

Plataforma única que serve **N câmaras municipais** (poder legislativo), cada uma com seu domínio, identidade visual e conteúdo, mas compartilhando o mesmo código e a mesma infraestrutura. **Fork de `portal-prefeitura`**: reaproveita toda a camada transversal (multi-tenancy RLS, auth, manifestações ESIC/Ouvidoria, transparência, IA, tema WCAG, CMS, LGPD, WhatsApp) e adiciona os módulos do domínio legislativo (Parlamentar, Sessões, Tramitação, Escola Legislativa, PSS).

> **Plano-mestre:** ver [`PLANEJAMENTO.md`](PLANEJAMENTO.md) — estratégia, roadmap de 8 fases e mapa de equivalência Laravel→nova plataforma.
