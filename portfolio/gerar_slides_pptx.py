# -*- coding: utf-8 -*-
"""
Gera a APRESENTAÇÃO do "Portal da Câmara" em .pptx (16:9), pronta para
...
Saída: portfolio/Apresentacao-Portal-Camara.pptx
"""
import os
...
def footer(s, dark=False):
    ...
    textbox(s, Inches(0.6), SH - Inches(0.45), Inches(12.1), Inches(0.3),
            [[("Lidera Tecnologia e Gestão  ·  Portal da Câmara  ·  " + SITE, 9, col, False, False)]],
            align=PP_ALIGN.LEFT)
...
textbox(s, Inches(1), Inches(3.0), Inches(11.3), Inches(1.6),
        [[("Portal da Câmara", 46, BRANCO, True, False)]], align=PP_ALIGN.CENTER)
textbox(s, Inches(1.4), Inches(4.3), Inches(10.5), Inches(1.0),
        [[("A plataforma digital completa, acessível e em conformidade com a lei para o seu Legislativo municipal.",
...
col1 = ["O desafio das câmaras hoje", "A plataforma e a arquitetura",
        "Modelo SaaS multi-tenant", "Módulos legislativos + transversais", "Conformidade legal nativa"]
col2 = ["Atendimento Omnichannel com IA (5 canais)", "Transparência: 100% PNTP (Diamante)",
        "Segurança, privacidade e LGPD", "App do Cidadão e IA Legislativa",
...
bullets(s, [
 "Produção legislativa dispersa: projetos, tramitação, votações e leis em sistemas e planilhas separados.",
 "Transparência e PNTP: difícil atingir e manter o selo da Atricon com dados do Legislativo espalhados.",
 "Prazos legais: e-SIC (LAI) e Ouvidoria (Lei 13.460) exigem controle que planilha não dá.",
 "Acessibilidade: a lei exige WCAG/eMAG e VLibras — a maioria dos sites de câmaras não cumpre.",
 "Custo e dependência: site, transparência, TV Câmara e sistema legislativo são contratos e fornecedores distintos.",
...
textbox(s, Inches(0.7), Inches(1.9), Inches(11.9), Inches(0.9),
        [[("Sessões plenárias, processo legislativo, parlamentares, TV Câmara, Escola Legislativa, "
           "transparência, ouvidoria/e-SIC, app mobile e IA legislativa — em um só produto, com a lei embutida.",
           16, CINZA_TX, False, False)]])
cards = [("Tudo integrado", "Módulos legislativos e transversais no mesmo login e dados", AZUL),
         ("Conforme a lei", "LAI · Lei 13.460 · LC 131/LRF · LGPD · WCAG", VERDE),
         ("100% PNTP", "Selo Diamante em transparência pública", AMARELO),
         ("Multi-tenant", "uma infra serve N câmaras, com identidade própria", AZUL_ESC)]
...
bullets(s, ["Frontend e app falam só com a API (gateway único).",
            "Dados isolados por RLS — câmara nunca vê dados de outra.",
...
# =====================================================================
s = slide(); header(s, "Modelo SaaS", "Uma plataforma, muitas câmaras")
pic_contain(s, os.path.join(ASSETS, "multitenant.png"), Inches(0.8), Inches(1.95),
            Inches(11.7), Inches(3.3), align="center")
bullets(s, ["Custo compartilhado entre as câmaras atendidas.",
...
# 7 — MÓDULOS
# =====================================================================
s = slide(); header(s, "Produto", "Módulos legislativos + transversais, em 6 áreas")
grupos = [("Atividade Legislativa", AZUL, "Projetos de lei · Tramitação · Votação nominal · Leis/normas · Iniciativa popular · Diário Oficial"),
          ("Sessões & Parlamentares", VERDE, "Pauta · Ata · Presença/frequência · Calendário · TV Câmara · Vereadores · Mesa Diretora · Comissões"),
          ("Escola Legislativa & PSS", RGBColor(0xB3,0x53,0x1D), "Cursos · Provas · Certificados com QR + validação · Fórum · PSS (editais/vagas/ranking/APLIC) · Eventos"),
          ("Cidadão & Participação", RGBColor(0x6f,0x42,0xc1), "Ouvidoria/e-SIC · Bot agêntico 24h · WhatsApp · Enquetes · Formulários · App do Cidadão · CMS/Notícias"),
          ("IA Legislativa", RGBColor(0x15,0x5F,0x8A), "Busca semântica em leis/projetos · Chatbot da câmara · Triagem de manifestações · Resumo de atas · OCR"),
          ("Conformidade & Segurança", RGBColor(0xE5,0x22,0x07), "Transparência/PNTP · LGPD self-service · Sigilo ouvidoria (RBAC+RLS) · Turnstile · WCAG/VLibras · Login gov.br")]
...
bullets(s, [
    ...
    "Instagram Direct — perfil profissional da camara.",
...
bullets(s, [
    ...
    "Fila por canal e por setor/comissao — historico completo.",
    "Multi-numero. Alerta de creditos de template WhatsApp.",
    "Chaves cifradas AES-256-GCM. Isolamento por camara (RLS).",
...
bullets(s, ["Prazos legais já configurados: e-SIC 20+10, Ouvidoria 30+30.",
            "Tramitação e votação nominal publicadas com base legal.",
...
textbox(s, Inches(0.9), Inches(5.85), Inches(11.5), Inches(0.6),
        [[("Sua câmara entra no ar já na nota máxima da transparência pública — "
           "Receita, Despesa, RH, Licitações, Contratos e produção legislativa em 100%.", 14, CINZA_TX, False, False)]],
        align=PP_ALIGN.CENTER)
...
bullets(s, [
 "Isolamento por RLS: cada consulta é restrita à câmara — impossível cruzar dados entre câmaras.",
 "Sigilo jurídico da ouvidoria: RBAC + RLS por papel — denúncias invisíveis até para o administrador da câmara.",
 "Autocadastro de usuário + solicitação de elevação por cargo; ouvidor exige aprovação da operadora e assina termo de sigilo.",
 "Escopo por setor/comissão: servidor só gerencia o conteúdo da sua área.",
...
# 11 — APP + IA
# =====================================================================
s = slide(); header(s, "Cidadão & IA", "App do Cidadão + IA Legislativa")
rect(s, Inches(0.7), Inches(2.0), Inches(5.85), Inches(4.5), CINZA_BG)
rect(s, Inches(0.7), Inches(2.0), Inches(5.85), Inches(0.5), AZUL)
textbox(s, Inches(0.9), Inches(2.05), Inches(5.5), Inches(0.45),
        [[("📱  App do Cidadão (mobile)", 16, BRANCO, True, False)]], anchor=MSO_ANCHOR.MIDDLE)
bullets(s, ["Acompanhe sessões, pauta, votação nominal e a TV Câmara.",
            "Consulte projetos de lei, tramitação e leis publicadas.",
            "Manifestações de ouvidoria/e-SIC e notificações (push).",
            "Login gov.br · tema da câmara (white-label)."],
        x=Inches(0.95), y=Inches(2.7), w=Inches(5.4), size=13.5, gap=10)
rect(s, Inches(6.75), Inches(2.0), Inches(5.85), Inches(4.5), CINZA_BG)
rect(s, Inches(6.75), Inches(2.0), Inches(5.85), Inches(0.5), VERDE)
textbox(s, Inches(6.95), Inches(2.05), Inches(5.5), Inches(0.45),
        [[("🤖  IA Legislativa", 16, BRANCO, True, False)]], anchor=MSO_ANCHOR.MIDDLE)
bullets(s, ["Busca semântica em leis e projetos (palavra + significado) + OCR de PDFs antigos.",
            "Chatbot da câmara treinável pelo gestor: artigos livres viram conhecimento da IA.",
            "Bot agêntico: abre protocolo e consulta tramitação por linguagem natural.",
            "Triagem de manifestações, resumo de atas e moderação automática."],
...
# 12 — CASO REAL
# =====================================================================
s = slide(); header(s, "Prova de entrega", "Caso real — Câmara de Barão de Melgaço (MT)")
...
nums = [("455", "notícias migradas"), ("1.180", "documentos migrados"),
        ("13", "setores/comissões publicados"), ("100%", "critérios PNTP (Diamante)")]
...
textbox(s, Inches(0.7), Inches(5.3), Inches(11.9), Inches(1.0),
        [[("Também migrados o institucional, a galeria e o acervo de leis/normas — com redirecionamentos "
...
# =====================================================================
s = slide(); header(s, "Implantação & Roadmap", "Roda onde a câmara precisar")
bullets(s, ["Nuvem gerenciada (Google Cloud / AWS) — provisionada por Terraform.",
            "On-premise (Windows / Linux / Docker) — no servidor da câmara.",
            "Migração do site atual sem perder histórico nem SEO.",
            "Treinamento das equipes e suporte continuado.",
            "Fases concluídas: Fundação · Transparência · Legislativo+CMS · Diário Oficial · IA Legislativa."],
...
textbox(s, Inches(1), Inches(2.6), Inches(11.3), Inches(1.0),
        [[("Vamos modernizar a sua câmara?", 34, BRANCO, True, False)]], align=PP_ALIGN.CENTER)
textbox(s, Inches(1.4), Inches(3.7), Inches(10.5), Inches(0.9),
        [[("Agende uma demonstração gratuita e veja o portal funcionando com os dados da sua câmara.",
...
textbox(s, Inches(1), SH - Inches(0.7), Inches(11.3), Inches(0.4),
        [[("Lidera Tecnologia e Gestão  ·  Investimento sob consulta, conforme o porte da câmara.",
           11, RGBColor(0xA9,0xB6,0xCC), False, True)]], align=PP_ALIGN.CENTER)

out = os.path.join(BASE, "Apresentacao-Portal-Camara.pptx")
prs.save(out)
